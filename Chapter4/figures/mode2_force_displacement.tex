  ...
  \begin{subfigure}{0.47\textwidth}
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \legend{geometric notch $d_{\text{critical}}=0.2$,geometric notch $d_{\text{critical}}=0.4$,geometric notch $d_{\text{critical}}=0.6$,geometric notch $d_{\text{critical}}=0.8$,initial damage $d_{\text{critical}}=0.2$,initial damage $d_{\text{critical}}=0.4$,initial damage $d_{\text{critical}}=0.6$,initial damage $d_{\text{critical}}=0.8$}
      \end{axis}
    \end{tikzpicture}
    \caption{}
    \label{fig: Chapter4/mode2_force_displacement_compare_dcrit}
  \end{subfigure}
  \caption{ Mode II force--displacement curves obtained using (a) spectral decomposition with two representations of the initial crack (b) spectral decomposition in conjunction with the contact split with different critical damage threshold on two representations of the initial crack. }
