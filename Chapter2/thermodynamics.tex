...
Let $\rho_0$ be the density in the reference configuration, $\btb$ be the distributed body force per unit mass, $\bfP$ be the first Piola-Kirchhoff stress, $\normal_0$ be the outward normal in the reference configuration, $u$ be the internal energy density, $k$ be the kinetic energy density, $\mathcal{P}^\text{ext}$ be the external power expenditure, $s$ be the entropy density, $q$ be the distributed heat source per unit mass, and $\bth$ be the heat flux. The densities $u$, $k$ and $s$ are all defined per unit volume in the reference configuration.
...
\begin{subequations}
  \begin{align}
     & \dot{\rho_0} = 0,                                                                                                                \\
     & \rho_0 \dot{\bs{\upphi}} = \divergence \bfP + \rho_0\btb,                                                                        \\
...
For convenience, the collection of thermodynamic state variables (working with the Helmholtz free energy), kinematic degrees of freedon, and internal variables are defined as
\begin{align}
...
\begin{align}
  \dot{\Lambda} = \{ \dot{\defgrad}, \dot{\defgrad}^p, \epdot, \dot{d}, \grad \dot{d} \}.
\end{align}
Recall that the local thermodynamic state is assumed to be depend only on $\defgrad$, $\bfZ$ and $s$, i.e.
\begin{align}
...
\begin{subequations}
  \begin{align}
    \bfP     & = \bfP^\text{eq}(\Lambda, T) + \bfP^\text{vis}(\dot{\defgrad}; \Lambda, T),       \\
    \bfT     & = \bfT^\text{eq}(\Lambda, T) + \bfT^\text{vis}(\dot{\defgrad}^p; \Lambda, T),     \\
    Y        & = Y^\text{eq}(\Lambda, T) + Y^\text{vis}(\epdot; \Lambda, T),                     \\
    f        & = f^\text{eq}(\Lambda, T) + f^\text{vis}(\dot{d}; \Lambda, T),                    \\
    \bs{\xi} & = \bs{\xi}^\text{eq}(\Lambda, T) + \bs{\xi}^\text{vis}(\grad\dot{d}; \Lambda, T),
  \end{align}
\end{subequations}
...
\begin{align}
  \dot{s} = -\dot{\psi}_{,T} = - \bfP^\text{eq}_{,T}:\dot{\defgrad} - \bfT^\text{eq}_{,T}:\dot{\defgrad}^p - Y^\text{eq}_{,T}\epdot - f^\text{eq}_{,T}\dot{d} - \bs{\xi}^\text{eq}_{,T}\cdot\grad d - \psi_{,TT} \dot{T}.
\end{align}
...
\begin{align}
  \delta_T = \bfP^\text{eq}_{,T}:\dot{\defgrad} + \bfT^\text{eq}_{,T}:\dot{\defgrad}^p + Y^\text{eq}_{,T}\epdot + f^\text{eq}_{,T}\dot{d} + \bs{\xi}^\text{eq}_{,T}\cdot\grad d,
\end{align}
e.g. $\bfP^\text{eq}_{,T}:\dot{\defgrad}$ is the dissipation accounting for thermoelastic effects, and $Y^\text{eq}_{,T}\epdot$ is the dissipation accounting for thermoplastic softening.
