...
\begin{align}
  ...
  \varphi(\dot{\Lambda}, \dot{s}, T, \grad T) &= \dot{k}(\dot{\bs{\upphi}}) + \dot{u}(\dot{\Lambda}, \dot{s}) + \Delta^*\left( \dfrac{T}{T^\text{eq}}\dot{\Lambda}, T^\text{eq} \right) - T\dot{s} - \chi(\btg), \label{eq: local energy density}
\end{align}
\end{subequations}
...
\begin{align}
    ...
    \varphi'(\dot{\Lambda}, \dot{s}, T, \grad T) &= \dot{k}(\dot{\bs{\upphi}}) + \dot{u}(\dot{\Lambda}, \dot{s}) + \Delta^*\left( \dot{\Lambda}, T \right) - T\dot{s} - \chi(\btg).
\end{align}
\end{subequations}
...
\begin{equation}
\begin{aligned}
    \varphi(\dot{\Lambda}, \dot{s}, T, \grad T) =&\ \rho_0\bta\cdot\dot{\bs{\upphi}} + \bfP^\text{eq}:\dot{\defgrad} + \bfT^\text{eq}:\dot{\defgrad}^p + Y^\text{eq}\epdot + f^\text{eq}\dot{d} + \bs{\xi}^\text{eq}\cdot\grad\dot{d} \\
...
\begin{equation}
\begin{aligned}
  L(\dot{\Lambda}, \dot{s}, T, \grad T) =&\ \int\limits_{\body_0} \left[ \rho_0\bta\cdot\dot{\bs{\upphi}} + \bfP^\text{eq}:\dot{\defgrad} + \bfT^\text{eq}:\dot{\defgrad}^p + Y^\text{eq}\epdot + f^\text{eq}\dot{d} \right.\\
  ...
  &\ - \int\limits_{\body_0} \rho_0 \btb \cdot \dot{\bs{\upphi}} \diff{V} - \int\limits_{\partial_t\body_0} \btt \cdot \dot{\bs{\upphi}} \diff{A} - \int\limits_{\partial_h\body_0} \bar{h}_n\ln\left( \dfrac{T}{T_0} \right) \diff{A} \\
